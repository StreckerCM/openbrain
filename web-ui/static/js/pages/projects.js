...
function ProjectDetail({ name }) {
    ...
    useEffect(() => {
        ...
        const fetchLinked = activeTab === 'knowledge'
            ? readKnowledge(`projects=cs.%7B${encodeURIComponent(decodedName)}%7D&status=eq.active&order=updated_at.desc&limit=50`)
            : readMemories(`projects=cs.%7B${encodeURIComponent(decodedName)}%7D&status=eq.active&order=updated_at.desc&limit=50`);
